...
public class DefaultHttpClient implements HttpClient {
  ...
  public static void initDefault(){
    Default = DefaultHttpClient.create ("bsf.system",HttpClientProperties.toMap());
  }
...
//    DnsResolver dnsResolver = SystemDefaultDnsResolver.INSTANCE;
    ...
    manager.setValidateAfterInactivity(initParams.getParams("ValidateAfterInactivity",int.class));
    ...
    RequestConfig defaultRequestConfig = RequestConfig.custom()
            ...
            .setConnectionRequestTimeout(initParams.getParams("ConnectionRequestTimeout",int.class))
            .build();

    //创建
    HttpClientBuilder httpClientBuilder = HttpClients.custom()
          //连接池不是共享模式,true
          .setConnectionManagerShared(initParams.getParams("ConnectionManagerShared",boolean.class));

    //定期回收空闲连接 60
    httpClientBuilder = httpClientBuilder.evictIdleConnections(initParams.getParams("EvictIdleConnectionsTime",int.class), TimeUnit.SECONDS);
    ...
    if(initParams.getParams("ConnectionTimeToLive",int.class)>0) {
      ...
      httpClientBuilder = httpClientBuilder.setConnectionTimeToLive(initParams.getParams("ConnectionTimeToLive",int.class), TimeUnit.SECONDS);
    }
    if(initParams.getParams("RetryCount",int.class)>0)
    {
      //设置重试次数，默认是3次，当前是禁用掉（根据需要开启） 0
      httpClientBuilder = httpClientBuilder.setRetryHandler(new DefaultHttpRequestRetryHandler(initParams.getParams("RetryCount",int.class), false));
    }

    //设置默认请求配置
    this.client = httpClientBuilder.setDefaultRequestConfig(defaultRequestConfig)
  ...
  {
    if(uri==null){throw new HttpException("Uri cannot be null");}
  }

  private void checkParmas(Params params)
  {
    if(params==null){throw new HttpException("Params cannot be null");}
  }

  public CloseableHttpResponse get(String uri,Params params, Callable.Func1<CloseableHttpResponse,CloseableHttpResponse> action) {
    checkUri(uri);
    HttpGet httpGet;
    if(params!=null)
    {
  ...
  public <T> T get(String uri, TypeReference<T> ref) {
    ...
    return JsonUtils.deserialize(rsp, ref);
  }
  ...
  public CloseableHttpResponse post(String uri,Params params, Callable.Func1<CloseableHttpResponse,CloseableHttpResponse> action) {
    checkUri(uri);
    HttpPost httpPost;
    if(params!=null)
    {
  ...
  public CloseableHttpResponse patch(String uri,Params params, Callable.Func1<CloseableHttpResponse,CloseableHttpResponse> action) {
    checkUri(uri);
    HttpPatch httpPatch;
    if(params!=null)
    {
  ...
  public CloseableHttpResponse put(String uri,Params params, Callable.Func1<CloseableHttpResponse,CloseableHttpResponse> action) {
    checkUri(uri);
    HttpPut httpPut;
    if(params!=null)
    {
  ...
  public CloseableHttpResponse delete(String uri,Params params, Callable.Func1<CloseableHttpResponse,CloseableHttpResponse> action) {
    checkUri(uri);
  ...
  public void close() {
      ...
      //从连接管理中移除
      try{ HttpClientManager.Default.remove(this);}
      catch (Exception e){
        exception =e;
      }
      if(exception!=null)
